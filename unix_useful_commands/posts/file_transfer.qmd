...
If you use an ssh-key (check [SSH keys and config file](./ssh_key_config.html)), you can run:


```{.bash}
scp -r HOST:/home/user/folder1 folder1
````

Just replace `HOST` with the name in the `Host` line in the `~/.ssh/config` file.