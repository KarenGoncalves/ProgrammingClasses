...
- `cp -v -r folder1 folder2`
	- Creates a copy of folder1 and names it folder2. 
	- The option `-v` (verbose) will make the computer print to the screen what it is doing (copied folder1/file1 to folder2/file1; copied folder1/file2 to folder2/file2; etc). 
...
*remove directory*: Use with the path to a folder to delete it FOREVER (same as `rm`). Only works if the folder is empty.

It is what `rm -r folder` uses to remove the folder. 
  - Once it removed all the files in the folder, it uses `rmdir folder`.